...
<html>
    <head>
        ...
        <link href='https://api.tiles.mapbox.com/mapbox-gl-js/v1.12.0/mapbox-gl.css' rel='stylesheet' /> <!--Stylesheet for map-->
        ...
        <script src='https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-geocoder/v4.2.0/mapbox-gl-geocoder.min.js'></script> <!--Mapbox API-->
        ...
        <div class = "row"> <!--container for map and location listings. uses bootstrap class for responsiveness-->
                ...
                <div id="map" class="map pad2">
                    <script>
                        ...
                        navigator.geolocation.getCurrentPosition(function(position) {
                            ...
                            //map gets centered on user's location.
                            ...
                            map.on('load', function() {
                                ...
                                var response = results; // This variable will store the JSON response body from the API
                                var output = "" 
                                var b = true;
                                var displayString = "";
                                for (var i = 0; i < response['features'].length; i++){//Searching for relevant information in the JSON response body
                                    for (var j =0; j < response['query'].length;j++){
                                        if (!JSON.stringify(!response['features'][i]['place_name'].includes(response['query'][j]))) { // if a result does not contain the searched location then do not add it to the output variable
                                            b = false;
                                        }
                                    ...
                                    if (b){
                                        var output = output + response['features'][i]['place_name'] +"<br></br>";// add the relevant result to the output variable
                                        console.log(response['features'][i]['place_name']);
                                        document.getElementById("printHere").innerHTML = output;// send the results to be displayed to the user
                                    }
            ...
            <div class = "column" id = "resultBar" >
                <div class="listing pad2">
                    ...
                    <br>
                    <p id="printHere" style = "font-family: 'Open Sans', sans-serif;"></p>  <!--The search results will be printed here-->
                </div>
            </div>
        </div>
    </div>
